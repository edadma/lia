...
object Util
{
	...
	def coprime( as: List[Int] ): Boolean = coprime( as.head, as.tail ) && (as.tail == Nil || coprime( as.tail ))
	...
	val mods = eqs map (_._2) toList
		...
		if (!coprime( mods )) sys.error( "crt: the moduli must all be pairwise coprime" )
